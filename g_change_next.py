...
def clean_industry_noise(s: str) -> str:
    """業種カラムに紛れ込む『レビュー/評価/件数／レビューなし』などのノイズを除去。"""
    if not s:
        return ""
    t = str(s)
    ...
    # 「Google のクチコミ」「口コミ」「クチコミ」「レビュー」「レビューなし」などを削除
    # （前後が「・」で区切られていてもOK）
    t = re.sub(r"(?:^|[・･])\s*(Google\s*の?\s*クチコミ|口コミ|クチコミ|レビューなし?|レビュー)\s*(?=[・･]|$)", "", t)

    # 「◯件のレビュー」「◯件の口コミ」などのパターン
    t = re.sub(r"[・･]?\s*\d+\s*件の?(レビュー|口コミ|クチコミ)\s*(?=[・･]|$)", "", t)

    # いったん「・」「･」で分割して、空要素を削除してから再結合
